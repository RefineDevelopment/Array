        ...
        if (event.getItem() != null && event.getAction().name().contains("RIGHT")) {
            ...
            switch (hotbarType) {
                ...
                case REMATCH_ACCEPT: {
                    ...
                    final RematchProcedure rematchProceedure= profile.getRematchData();
                    if (rematchProceedure.isReceive()) {
                        rematchProceedure.accept();
                    }
                    else {
                        if (rematchProceedure.isSent()) {
                            player.sendMessage(Locale.ERROR_REMATCHSENT.toString());
                            return;
                        }
                        rematchProceedure.request();
                    }
                    break;
                }
                default: {}

...
public class SkyWarsRoundStartTask extends SkyWarsTask {
    ...
    public void onRun() {
        if (getTicks() >= 3) {
            ...
            this.getSkyWars().getPlayers().forEach(PlayerUtil::allowMovement);
            ...
            Bukkit.getScheduler().runTaskAsynchronously(Array.getInstance(), () -> {
                int x = 0;
                Random random = new Random();
                OUTTER_LOOP: for ( SkywarsChests chest : SkywarsChests.chests.values()) {

    ...
    public void onDeath(EntityDeathEvent event) {
        ...
        if (profile.isInSkyWars()) {
            if (event.getEntity() instanceof Player) {
                ...
                Bukkit.getScheduler().runTaskLaterAsynchronously(Array.getInstance(), () -> event.getDrops().clear(), 10 * 20);
            }
        }
    }
                ...
                if (skyWars != null) {
                    if (skyWars.isWaiting() || skyWars.getState().equals(SkyWarsState.ROUND_ENDING)) {
                        if (event.getCause().equals(EntityDamageEvent.DamageCause.VOID)) {
                            event.getEntity().teleport(LocationUtil.deserialize(Array.getInstance().getSkyWarsManager().getSkyWarsSpectators().get(0)));
                        }
                        event.setCancelled(true);
                    }
                }

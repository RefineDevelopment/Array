...
@CommandMeta(label = "arena removekit", permission = "array.dev")
public class ArenaRemoveKitCommand {
    ...
    public void execute(Player player, @CPL("arena") Arena arena, @CPL("kit") Kit kit) {
        ...
        if (arena.getKits().contains(kit.getName())) {
            ...
            player.sendMessage(CC.translate("&8[&b&lArray&8] &a") +  "Successfully removed the kit " + kit.getName() + " from " + arena.getName());
            arena.save();
        }
    }

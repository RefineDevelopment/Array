...
public class RobotLogic extends BukkitRunnable {
    ...
    private final RobotType difficulty;
    ...
    public RobotLogic(Robot robot, List<UUID> players, RobotType difficulty) {
        this.robot = robot;
        this.target = null;
    ...
    public void initiateLogic() {
        ...
        this.runTaskTimerAsynchronously(plugin, 60L, delay);
        if (robot.getKit() != null && robot.getKit().getGameRules().isStickSpawn()) {
            new BukkitRunnable() {
                int count = 3*2+2;
                ...
                public void run() {
                    ...
                    try {
                        robot.getPlayer().teleport(robot.getSpawnLocation());
                    }catch(Exception e){}
                }
    ...
    private void giveKit(Kit kit) {
        kit.applyToPlayer(robot.getPlayer());
        SpigotHook.getKnockbackType().appleKitKnockback(robot.getPlayer(), kit);
        this.kit = true;
    }
    ...
    private void useGoldenApple() {
        ...
        if (gapple != null) {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        try {
                            ...
                            Array.logger("&cError! Invalid Citizens Version!");
                        }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (robot.getPlayer() != null) {
                                ...
                                }.runTaskLater(plugin, (random.nextInt(4) + 2));
            ...
            }.runTaskLater(plugin, (this.random.nextInt(2) + 1));
        }
    }
            ...
            new BukkitRunnable() {
                public void run() {
                    ...
                    if (!robot.getPlayer().isDead()) {
                        ...
                        }.runTaskLater(plugin, (random.nextInt(1) + 1));

                    }
                }
            }.runTaskLater(plugin, (this.random.nextInt(1) + 1));
            return true;
        }
        return false;
    }
                ...
                public void run() {
                    if (robot.getNpc() != null && robot.isSpawned() && robot.getNpc().getNavigator() != null) {
                        ...
                        if (this.counter == 0 || Math.abs(robot.getPlayer().getLocation().getPitch() - 90.0f) < 50.0f) {
                            ...
                            }.runTaskLater(plugin, (random.nextInt(12) + 8));
                        }
                    } else {
                        this.cancel();
                    }
    ...
    @Override
    public void run() {
        if (this.players == null || this.players.isEmpty() || robot.isDeSpawned()) {
            this.cancel();
            return;
        }
        ...
        if (robot.isSpawned() && robot.getPlayer() != null) {
            ...
            if (!this.navigation) {
                ...
                if (this.difficulty == RobotType.HARD) {
                    robot.getNpc().getNavigator().getLocalParameters().speedModifier(1.33f);
                } else if (this.difficulty == RobotType.EXPERT) {
                ...
                robot.getNpc().getNavigator().getLocalParameters().stuckAction((npc, navigator) -> false);
            }

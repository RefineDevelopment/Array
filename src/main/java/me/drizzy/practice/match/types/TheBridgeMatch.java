...
public class TheBridgeMatch extends Match {
    ...
    public void onPortalEnter(Player player) {
        if (this.getState() == MatchState.ENDING) return;

        if (LocationUtils.isTeamPortal(player)) {
            new BridgePlayerTask(this, player).run();
            player.sendMessage(Locale.MATCH_BRIDGE_WRONG_PORTAL.toString());
            return;
        }

        if (this.getCaughtPlayers().contains(player)) return;
        ...
        for ( TeamPlayer teamPlayer : this.getTeamPlayers() ) {
            ...
            other.sendMessage(Locale.MATCH_BRIDGE_SCORED.toString()
                    .replace("<relation_color_scored>", this.getRelationColor(other, player).toString())
                    .replace("<scored_name>", player.getName()));

            teamPlayer.getPlayer().teleport(teamPlayer.getPlayerSpawn());
        }
        this.handleDeath(this.getOpponentPlayer(player), null, false);
    }

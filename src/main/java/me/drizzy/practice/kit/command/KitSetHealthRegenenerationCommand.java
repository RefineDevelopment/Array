...
public class KitSetHealthRegenenerationCommand {
    public void execute(Player player, @CPL("kit") Kit kit) {
        if (kit == null) {
            ...
            kit.save();
            player.sendMessage(CC.translate("&8[&c&lArray&8] &7Updated health-regeneration mode for &c" + kit.getName() +  " &7to &c" + (kit.getGameRules().isHealthRegeneration() ? "true!" : "false!")));
        }
    }
}
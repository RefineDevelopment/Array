...
@CommandMeta(label={"kit showhealth"}, permission = "practice.dev")
public class KitShowHealthCommand {
    public void execute(Player player, @CPL("kit") Kit kit) {
        if (kit == null) {
            ...
            kit.save();
            player.sendMessage((CC.translate("&8[&b&lArray&8] &a")) + "Kit showhealth set to " + (kit.getGameRules().isShowHealth() ? "true!" : "false!"));
        }
    }
}

...
public class Archer extends ArmorClass implements Listener {
	...
	public static TObjectLongMap<UUID> archerJumpCooldowns = new TObjectLongHashMap<UUID>();
		...
		if (((entity instanceof Player)) && ((damager instanceof Arrow))) {
			...
			if ((source instanceof Player)) {
				...
				if (true) {
					...
					shooter.sendMessage(me.array.ArrayPractice.util.Color.translate("&e[&9Arrow Range &e(&c" + String.format("%.1f", Double.valueOf(distance)) + "&e)] " + "&6Marked " + damaged.getName() + " &6for 6 seconds. &9&l(" +  heartdamage+" heart)"));

					damaged.sendMessage(CC.RED + "Marked! " + (CC.YELLOW) + "An archer has shot you and marked you (+25% damage) for 10 seconds " + ChatColor.RED + shooter.getName() + ChatColor.YELLOW + " from " + ChatColor.RED + String.format("%.1f", Double.valueOf(distance)) + ChatColor.YELLOW + " blocks away.");

...
public class ArmorClassManager implements Listener {
    ...
    private Map<UUID, ArmorClass> equippedClassMap = new HashMap<>();

    protected Map<UUID, ArmorClass> classWarmups = new HashMap<>();

    private List<ArmorClass> pvpClasses = new ArrayList<>();
            ...
            public void run() {
                for(Player player : Bukkit.getOnlinePlayers()){
                    ...
                    if (match != null && match.isHCFMatch()) {
                        Bukkit.getScheduler().runTask(Array.get(), () -> {
                            attemptEquip(player);
                        });
                    }
                }
    ...
    public void attemptEquip(Player player) {
        ArmorClass current = Array.get().getArmorClassManager().getEquippedClass(player);
        if (current != null) {
            if (current.isApplicableFor(player)) {
                return;
            }

            Array.get().getArmorClassManager().setEquippedClass(player, null);
        ...
        Collection<ArmorClass> pvpClasses = Array.get().getArmorClassManager().getPvpClasses();
        for (ArmorClass pvpClass : pvpClasses) {
            if (pvpClass.isApplicableFor(player)) {
                Array.get().getArmorClassManager().setEquippedClass(player, pvpClass);
                break;
            }
        }
    }

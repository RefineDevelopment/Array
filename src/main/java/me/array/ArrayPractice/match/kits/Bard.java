package me.array.ArrayPractice.match.kits;
...
import me.array.ArrayPractice.match.kits.utils.bard.BardData;
import me.array.ArrayPractice.match.kits.utils.bard.EffectData;
...
public class Bard extends ArmorClass implements Listener {
    ...
    private static long BUFF_COOLDOWN_MILLIS = TimeUnit.SECONDS.toMillis(10L); // time in milliseconds for Bard buff cooldowns
    private static int TEAMMATE_NEARBY_RADIUS = 25;
    private static long HELD_REAPPLY_TICKS = 20L;
    private Map<UUID, BardData> bardDataMap = new HashMap<>();
    private Map<Material, EffectData> bardEffects = new EnumMap<>(Material.class);
    private Array plugin;
    private TObjectLongMap<UUID> msgCooldowns = new TObjectLongHashMap<>();
    ...
    public void onItemHeld(PlayerItemHeldEvent event) {
        ...
        if (match != null && (match.isHCFMatch() || match.isKoTHMatch())) {
            ...
            ArmorClass equipped = plugin.getArmorClassManager().getEquippedClass(player);
        ...
        if (match != null && (match.isHCFMatch() || match.isKoTHMatch())) {
            ...
            if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
                ...
                if (bardData != null) {
                    ...
                    player.sendMessage(Color.translate("&cYou have just used a &lBard Buff &cthat cost you " + bardEffect.energyCost + " &cof your Energy."));
                }
            }
        }
    }
    ...
    private boolean canUseBardEffect(Player player, BardData bardData, EffectData bardEffect, boolean sendFeedback) {
        ...
        if (bardEffect.energyCost > currentEnergy) {
            errorFeedback = Color.translate("&cYou do not have enough energy for this! You need " + bardEffect.energyCost + " energy, but you only have " + currentEnergy);
        }

...
import me.array.ArrayPractice.event.impl.ffa.player.FFAPlayer;
...
public class Profile
{
    ...
    public Profile(final UUID uuid) {
        ...
        this.kitData = new HashMap<Kit, ProfileKitData>();
        this.enderpearlCooldown = new Cooldown(0L);
        this.sentDuelRequests = new HashMap<UUID, DuelRequest>();
        this.uuid = uuid;
    ...
    public boolean isSpectating() {
        return this.state == ProfileState.SPECTATE_MATCH && (this.match != null || this.sumo != null || this.brackets != null || this.ffa != null || this.juggernaut != null || this.parkour != null || this.wipeout != null || this.skyWars != null || this.spleef != null || this.infected != null);
    }
    ...
    public boolean isInJuggernaut() {
        return this.state == ProfileState.IN_EVENT && this.juggernaut != null;
    }
    ...
    public boolean isInWipeout() {
        return this.state == ProfileState.IN_EVENT && this.wipeout != null;
    }
    
    public boolean isInSkyWars() {
        return this.state == ProfileState.IN_EVENT && this.skyWars != null;
    }
    ...
    public boolean isInInfected() {
        return this.state == ProfileState.IN_EVENT && this.infected != null;
    }
    ...
    public void checkForHotbarUpdate() {
        ...
        if (this.isInLobby() && !this.kitEditor.isActive()) {
            ...
            if (this.rematchData != null) {
                ...
                if (System.currentTimeMillis() - this.rematchData.getTimestamp() >= 30000L) {
                    ...
                    if (!profile.isInLobby() && !profile.isInQueue()) {
                        ...
                        final int requestSlot = player.getInventory().first((ItemStack)Hotbar.getItems().get(HotbarItem.REMATCH_REQUEST));
                        if (requestSlot != -1) {
                            update = true;
                        }
                    }
            ...
            final boolean activeEvent = (Array.get().getSumoManager().getActiveSumo() != null && Array.get().getSumoManager().getActiveSumo().isWaiting()) || (Array.get().getBracketsManager().getActiveBrackets() != null && Array.get().getBracketsManager().getActiveBrackets().isWaiting()) || (Array.get().getFfaManager().getActiveFFA() != null && Array.get().getFfaManager().getActiveFFA().isWaiting()) || (Array.get().getJuggernautManager().getActiveJuggernaut() != null && Array.get().getJuggernautManager().getActiveJuggernaut().isWaiting()) || (Array.get().getParkourManager().getActiveParkour() != null && Array.get().getParkourManager().getActiveParkour().isWaiting()) || (Array.get().getWipeoutManager().getActiveWipeout() != null && Array.get().getWipeoutManager().getActiveWipeout().isWaiting()) || (Array.get().getSkyWarsManager().getActiveSkyWars() != null && Array.get().getSkyWarsManager().getActiveSkyWars().isWaiting()) || (Array.get().getSpleefManager().getActiveSpleef() != null && Array.get().getSpleefManager().getActiveSpleef().isWaiting()) || (Array.get().getInfectedManager().getActiveInfected() != null && Array.get().getInfectedManager().getActiveInfected().isWaiting());
            final int eventSlot = player.getInventory().first((ItemStack)Hotbar.getItems().get(HotbarItem.EVENT_JOIN));
    ...
    public void refreshHotbar() {
        ...
        if (player != null) {
            ...
            else if (this.isInJuggernaut()) {
                if (this.getJuggernaut().getEventPlayer(player).getState().equals(JuggernautPlayerState.ELIMINATED)) {
                    PlayerUtil.spectator(player);
                }
            ...
            else if (this.isInWipeout()) {
                if (this.getWipeout().getEventPlayer(player).getState().equals(WipeoutPlayerState.ELIMINATED)) {
                    PlayerUtil.spectator(player);
                }
            ...
            else if (this.isInInfected()) {
                if (this.getInfected().getEventPlayer(player).getState().equals(InfectedPlayerState.ELIMINATED)) {
                    PlayerUtil.spectator(player);
                }
    ...
    public void handleVisibility(final Player player, final Player otherPlayer) {
        ...
        if (this.state == ProfileState.IN_LOBBY || this.state == ProfileState.IN_QUEUE) {
            ...
            else if (this.juggernaut != null) {
                final JuggernautPlayer juggernautPlayer = this.juggernaut.getEventPlayer(otherPlayer);
            ...
            else if (this.wipeout != null) {
                final WipeoutPlayer wipeoutPlayer = this.wipeout.getEventPlayer(otherPlayer);
            ...
            else if (this.infected != null) {
                final InfectedPlayer infectedPlayer = this.infected.getEventPlayer(otherPlayer);
    ...
    public void handleVisibility() {
        ...
        if (player != null) {
            ...
            }.runTaskAsynchronously((Plugin) Array.get());
        }
    }
    ...
    public void load() throws Exception {
        ...
        if (essentials == null) {
            ...
            essentialsDocument.put("nick", (Object)null);
            document.put("essentials", (Object)essentialsDocument);
        }
        else {
    ...
    public void save() {
        ...
        document.put("globalElo", (Object)this.globalElo);
        final Document essentialsDocument = new Document();
        essentialsDocument.put("nick", (Object)this.essentials.getNick());
        document.put("essentials", (Object)essentialsDocument);
        final Document optionsDocument = new Document();
        optionsDocument.put("showScoreboard", (Object)this.options.isShowScoreboard());
        optionsDocument.put("allowSpectators", (Object)this.options.isAllowSpectators());
        optionsDocument.put("receiveDuelRequests", (Object)this.options.isReceiveDuelRequests());
        document.put("options", (Object)optionsDocument);
        ...
        for (final Map.Entry<Kit, ProfileKitData> entry : this.kitData.entrySet()) {
            ...
            kitDocument.put("elo", (Object)entry.getValue().getElo());
            kitDocument.put("rankedWon", (Object)entry.getValue().getRankedWon());
            kitDocument.put("rankedLost", (Object)entry.getValue().getRankedLost());
            kitDocument.put("unrankedWon", (Object)entry.getValue().getUnrankedWon());
            kitDocument.put("unrankedLost", (Object)entry.getValue().getUnrankedLost());
            kitStatisticsDocument.put(entry.getKey().getName(), (Object)kitDocument);
        }
        document.put("kitStatistics", (Object)kitStatisticsDocument);
        ...
        for (final Map.Entry<Kit, ProfileKitData> entry2 : this.kitData.entrySet()) {
            ...
            kitsDocument.put(entry2.getKey().getName(), (Object)kitsArray.toString());
        }
        document.put("loadouts", (Object)kitsDocument);
    ...
    public static void init() {
        ...
        }.runTaskTimerAsynchronously((Plugin) Array.get(), 10L * 40L * 5L, 10L * 40L * 5L);
        loadAllProfiles();
        new BukkitRunnable() {
            public void run() {
        ...
        }.runTaskTimerAsynchronously((Plugin) Array.get(), 30L * 60L * 5L, 30L * 60L * 5L);
        new BukkitRunnable() {
            public void run() {
                Profile.loadGlobalLeaderboards();
            }
        }.runTaskTimerAsynchronously((Plugin) Array.get(), 20L, 600L);
        new BukkitRunnable() {
            public void run() {
                for (final Profile profile : Profile.getProfiles().values()) {
        ...
        }.runTaskTimerAsynchronously((Plugin) Array.get(), 60L, 60L);
    }
    ...
    public static void loadGlobalLeaderboards() {
        ...
        for (final Document document : getAllProfiles().find().sort(Sorts.descending("globalElo")).limit(10).into(new ArrayList<Document>())) {
    ...
    public void setGlobalElo(final int globalElo) {
    ...
    public void setJuggernaut(final Juggernaut juggernaut) {
    ...
    public Wipeout getWipeout() {
        return this.wipeout;
    }
    
    public void setWipeout(final Wipeout wipeout) {
    ...
    public void setSkyWars(final SkyWars skyWars) {
    ...
    public void setInfected(final Infected infected) {
    ...
    public void setLastMessager(final Player lastMessager) {
        this.lastMessager = lastMessager;
    }
    
    static {
        Profile.profiles = new HashMap<UUID, Profile>();
        Profile.globalEloLeaderboards = new ArrayList<KitLeaderboards>();
    }
}

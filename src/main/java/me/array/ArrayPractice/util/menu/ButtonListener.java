...
import me.array.ArrayPractice.util.menu.pagination.PaginatedMenu;
...
public class ButtonListener implements Listener {
		...
		if (openMenu != null) {
			...
			if (openMenu.getButtons().containsKey(event.getSlot())) {
				...
				if (event.isCancelled()) {
					Bukkit.getScheduler().runTaskLater(Array.get(), () -> player.updateInventory(), 1L);
				}
			} else {
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		player.setMetadata("scanglitch", new FixedMetadataValue(Array.get(), true));
	}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (player.hasMetadata("scanglitch")) {
			player.removeMetadata("scanglitch", Array.get());

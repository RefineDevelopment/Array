...
public class Bard extends PvPClass implements Listener {
            ...
            int lastEnergy;
                ...
                public void run() {
                    ...
                    if (energy != 0 && energy != lastEnergy && (energy % 10 == 0 || lastEnergy - energy - 1 > 0 || energy == BardData.MAX_ENERGY)) {
                        lastEnergy = energy;
                        player.sendMessage(xyz.refinedev.practice.Locale.HCF_BARD_ENERGY.toString().replace("<energy>", String.valueOf(energy)));
                    }
                }
        ...
        if (match != null && match.isHCFMatch()) {
            ...
            if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
                ...
                if (bardData != null) {
                    ...
                    player.sendMessage(xyz.refinedev.practice.Locale.HCF_BARD_BARDBUFF.toString().replace("<cost>", String.valueOf(bardEffect.energyCost)));
                }
            }
        }
    }
    ...
    private boolean canUseBardEffect(Player player, BardData bardData, EffectData bardEffect, boolean sendFeedback) {
        ...
        if (bardEffect.energyCost > currentEnergy) {
            errorFeedback = xyz.refinedev.practice.Locale.HCF_BARD_NOTENOUGHENERGY.toString()

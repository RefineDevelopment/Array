...
public class SumoTeamStartTask extends EventStartTask {

    private final SumoTeam event;
    
    public SumoTeamStartTask(SumoTeam event) {
       super(event);
       this.event = event;
    }
        ...
        if (this.getEvent().getPlayers().size() == this.getEvent().getMaxPlayers() || (getTicks() >= 30 && this.getEvent().getPlayers().size() >= 2)) {
            if (this.getEvent().getCooldown() == null) {
                ...
                if (this.getEvent().getCooldown().hasExpired()) {
                    ...
                    this.getEvent().setEventTask(new SumoTeamRoundStartTask(event));
                }
            }
        }
